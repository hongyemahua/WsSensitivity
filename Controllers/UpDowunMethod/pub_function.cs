...
namespace WsSensitivity.Controllers.UpDowunMethod
{
    class pub_function
    {
        ...
        public static void get_zhixinqujian(double gailv, double zxsp, int test_n, string fenbu, double favg, double fsigma, double fsigmaavg, double fsigmasigma, out double xiangyingdian, out double xiangyingdian_shangxian, out double xiangyingdian_xiaxian, out double junzhishangxian, out double junzhixiaxian, out double biaozhunchashangxian, out double biaozhunchaxiaxian, out double xiangyingdian_dance_shangxian, out double xiangyingdian_dance_xiaxian)
        {
            ...
            if (fenbu == "正态分布幂")
            {
                xiangyingdian = Math.Pow(Math.Pow(favg, 1) + pub_function.qnorm(fq) * fsigma, 1 / 1);

                // 置信上限：
                xiangyingdian_shangxian = Math.Pow(Math.Pow(favg, 1) + pub_function.qnorm(fq) * fsigma + Tfw * Math.Sqrt(Math.Pow(fsigmaavg, 2) + Math.Pow(fsigmasigma, 2) * Math.Pow(pub_function.qnorm(fq), 2)), 1 / 1);

                // 置信下限：
                xiangyingdian_xiaxian = Math.Pow(Math.Pow(favg, 1) + pub_function.qnorm(fq) * fsigma - Tfw * Math.Sqrt(Math.Pow(fsigmaavg, 2) + Math.Pow(fsigmasigma, 2) * Math.Pow(pub_function.qnorm(fq), 2)), 1 / 1);
                xiangyingdian_dance_shangxian = Math.Pow(Math.Pow(favg, 1) + pub_function.qnorm(fq) * fsigma + Tfw_dance * Math.Sqrt(Math.Pow(fsigmaavg, 2) + Math.Pow(fsigmasigma, 2) * Math.Pow(pub_function.qnorm(fq), 2)), 1 / 1);
                xiangyingdian_dance_xiaxian = Math.Pow(Math.Pow(favg, 1) + pub_function.qnorm(fq) * fsigma - Tfw_dance * Math.Sqrt(Math.Pow(fsigmaavg, 2) + Math.Pow(fsigmasigma, 2) * Math.Pow(pub_function.qnorm(fq), 2)), 1 / 1);

                // 均值置信上限
                junzhishangxian = Math.Pow(Math.Pow(favg, 1) + Tfw * fsigmaavg, 1 / 1);

                // 均值置信下限
                junzhixiaxian = Math.Pow(Math.Pow(favg, 1) - Tfw * fsigmaavg, 1 / 1);
            ...
            if (fenbu == "逻辑斯谛分布幂")
            {
                xiangyingdian = (Math.Pow(Math.Pow(favg, 1) + pub_function.qlogis(fq) * fsigma, 1 / 1));


                // 置信上限：
                xiangyingdian_shangxian = Math.Pow(Math.Pow(favg, 1) + pub_function.qlogis(fq) * fsigma + Tfw * Math.Sqrt(Math.Pow(fsigmaavg, 2) + Math.Pow(fsigmasigma, 2) * Math.Pow(pub_function.qlogis(fq), 2)), 1 / 1);

                // 置信下限：
                xiangyingdian_xiaxian = Math.Pow(Math.Pow(favg, 1) + pub_function.qlogis(fq) * fsigma - Tfw * Math.Sqrt(Math.Pow(fsigmaavg, 2) + Math.Pow(fsigmasigma, 2) * Math.Pow(pub_function.qlogis(fq), 2)), 1 / 1);

                xiangyingdian_dance_shangxian = Math.Pow(Math.Pow(favg, 1) + pub_function.qlogis(fq) * fsigma + Tfw_dance * Math.Sqrt(Math.Pow(fsigmaavg, 2) + Math.Pow(fsigmasigma, 2) * Math.Pow(pub_function.qlogis(fq), 2)), 1 / 1);
                xiangyingdian_dance_xiaxian = Math.Pow(Math.Pow(favg, 1) + pub_function.qlogis(fq) * fsigma - Tfw_dance * Math.Sqrt(Math.Pow(fsigmaavg, 2) + Math.Pow(fsigmasigma, 2) * Math.Pow(pub_function.qlogis(fq), 2)), 1 / 1);
                // 均值置信上限
                junzhishangxian = Math.Pow(Math.Pow(favg, 1) + Tfw * fsigmaavg, 1 / 1);
